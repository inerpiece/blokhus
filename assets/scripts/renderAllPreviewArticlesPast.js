...
category.addEventListener('change', function () {
    ...
    //The below commented script is for optimizing categories
    // let categoriesObj = {
...
months.addEventListener('change', function () {
    ...
    let monthNamesLong = new Array;
    monthNamesLong = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    monthNames = ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"];
    for (let index = 0; index < allArticles.length; index++) {
        for (let j = 0; j < monthNames.length; j++) {
            if (option == monthNamesLong[j]) {
                if (allArticles[index].classList.contains(monthNames[j])) {
                    allArticles[index].classList.remove('hidden');
                } else {
                    allArticles[index].classList.add('hidden');
                }
            }
        }
        console.log("Month names short " + monthNames[index] + "Month names long " + monthNamesLong[index])
    }
    // allArticles.forEach(individualArticle => {
